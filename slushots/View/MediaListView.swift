    ...
    var body: some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 16) {
                let filteredIndices = (0..<viewModel.numberOfTracks).filter { index in
                    let info = viewModel.trackInfo(at: index)
                    return searchText.isEmpty || info.song.lowercased().contains(searchText.lowercased()) || info.artist.lowercased().contains(searchText.lowercased())
                }
